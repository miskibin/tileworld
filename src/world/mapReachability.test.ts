...
// A foot/apron tile of every biome + every ork camp (the mountain cores are
// deliberately cliffed off now, so we assert the player can reach the range, not
// necessarily summit it). Snapped to the nearest standable, prop-free tile.
const TARGETS: Record<string, [number, number]> = {
  'snow massif (NW)': [40, 34],
  'desert (NE)': [100, 30],
  'forest (SW)': [40, 72],
  'forest (SE)': [108, 82],
  'swamp (S)': [72, 88],

...
**Headless screenshots.** The preview/screenshot tooling runs Chrome without a GPU, so WebGL falls back to software (SwiftShader). The full scene's post-processing stack (N8AO + GodRays + Bloom + SMAA) makes one software-rendered frame take longer than the 30s screenshot budget — `eval` works but `screenshot` times out. Append **`?capture`** (or `?lite`) to the preview URL to enter *capture mode* ([renderMode.ts](src/world/renderMode.ts)): it drops the EffectComposer stack and pins `dpr` to 1, cutting frame cost enough to screenshot reliably (the scene still renders, just without the cinematic grade). `scripts/capture-probe.mjs` reproduces the no-GPU condition with Playwright and verifies both paths.
